...
QPHash::operator()(const uint64_t &k, size_t iter) const
{
    ...
    //      a collision happened.
    //Remember: m is two power and c1= c2 = 1/2.
    (iter == 0) ? (ret_v = universal_hash(k, m_, p_, a_, b_)) : (ret_v = (size_t)(universal_hash(k, m_, p_, a_, b_) + (0.5*iter) + (0.5*iter*iter)) % m_);
    //
    return ret_v;
}
...
RPHash::operator()(const uint64_t &k, size_t iter) const
{
    ...
    // Hint: you could save the first value to avoid recompute it when
    //       a collision happened.
    (iter == 0) ? (ret_v = universal_hash(k, m_, p_, a_, b_)) : (ret_v = (universal_hash(k, m_, p_, a_, b_) + iter*c_) % m_);
    //
    return ret_v;
}
